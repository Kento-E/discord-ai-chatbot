...
**重要**: PRがマージされた後、新しいPRを自動で作成することはできません。

フォローアップタスク（リンターツール導入、Issueテンプレート更新など）に取り組む場合は、手動でIssueとPRを作成する必要があります。

詳細な手順については、[PRマージ後のワークフローガイド](../../docs/POST_MERGE_WORKFLOW.md)を参照してください。
