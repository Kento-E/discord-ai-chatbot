...
#### ⚠️ Copilot向け特別指示（必須・例外なし）

> **`report_progress` を呼び出す前に、必ず以下のリンターコマンドを実行すること。**
> **この手順を省略してはならない。** `report_progress` は pre-commit フックをバイパスするため、
> 手動実行しない限りフォーマットエラーがそのままコミットされ、CIが失敗する。

**基本方針: 変更したファイルのみをリンターで検証**

```bash
# report_progress を呼ぶ直前に必ず実行する
...
- [ ] **⚠️（Copilot）`report_progress` 前にリンター（black, isort, autoflake, flake8）を実行したか（必須・例外なし）**
- [ ] リンターでエラーがないか確認したか
