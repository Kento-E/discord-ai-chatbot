...
BotはLLM（Google Gemini API）を使用し、過去のメッセージを文脈として自然で創造的な返信を生成します。
...
詳細は[LLM API統合ガイド](LLM_API_SETUP.md)を参照してください。
...
- **LLMモード**: Google Gemini APIを使用した応答生成（有効/設定不足）
- 知識データの状態（利用可能/未生成）
- AIエージェント機能の状態（有効/無効）

### 応答の特徴

BotはLLM（Google Gemini API）を使用して、過去のメッセージを文脈として自然な返信を生成します：
...
**応答**: 過去の類似メッセージを文脈として、LLMが自然な返信を生成します。

**入力**: `@YourBot おはよう`

**応答**: 過去の挨拶メッセージを参考に、LLMが適切な挨拶を返します。

**入力**: `@YourBot Pythonのインストール方法は？`

**応答**: 過去の関連メッセージを基に、LLMが詳細な説明を生成します。
