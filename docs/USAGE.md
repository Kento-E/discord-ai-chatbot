...
- 全チャンネルの全メッセージを取得（上限なし）
- Botのメッセージは除外
- `EXCLUDED_CHANNELS`で指定したチャンネルは除外（オプション）
- SQLiteデータベース（`data/knowledge.db`）に保存
- 既存メッセージはスキップ（増分更新）

データベース機能の詳細は[データベース管理ガイド](DATABASE.md)を参照してください。
...
✅ データベースへの保存が完了しました: /path/to/data/knowledge.db
