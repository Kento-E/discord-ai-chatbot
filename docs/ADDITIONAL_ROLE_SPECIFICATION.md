...
この機能により、設定ファイル（`config/prompts.toml`）のベースプロンプトに加えて、環境変数を通じて追加の役割や性格をチャットボットに指定できます。
...
```
<config/prompts.toml のベースシステムプロンプト>
...
1. システムプロンプト
   + 【追加の役割・性格】（設定されている場合）
   + 応答生成ルール（config/prompts.tomlのllm_response_instruction）
