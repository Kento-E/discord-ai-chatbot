...
- **保存先**: `data/knowledge.db`

### 2. 増分更新

既存のメッセージIDをチェックし、新規メッセージのみを追加します。

- **利点**: 2回目以降の実行が高速
- **実装**: メッセージIDによる重複チェック
- **動作**: 
  1. Discordから全メッセージを取得
  2. 既存IDと照合
  3. 新規メッセージのみ挿入
...
SQLiteはファイルベースのデータベースで、追加のサービス契約が不要です。

- **利点**: GitHub Actionsで追加コストなし
- **実装**: データベースファイルを暗号化してGitHub Releaseに保存
- **保存方法**: 既存の暗号化フローを活用
